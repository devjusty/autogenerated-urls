...
The `Release plugin` GitHub Actions workflow runs when a tag beginning with `v` is pushed. It verifies that the tag matches the plugin header version, builds a production-only ZIP, and attaches it to a GitHub Release.

1. Update `Version:` in `autogenerated-urls.php`.
...
The resulting release asset is named `autogenerated-urls-1.2.1.zip` and installs into a single `autogenerated-urls/` plugin directory. Development dependencies, tests, editor settings, skills, and local environment configuration are excluded.
